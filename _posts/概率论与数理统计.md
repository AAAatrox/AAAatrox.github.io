...
设$X\_1,X\_2...X\_n...$为一列随机变量,若存在随机变量$X$,使得任意给定的$\varepsilon > 0$,$\lim\_{n\to\infty}P(|X\_n-X|\ge\varepsilon)=0$或等价地$\lim\_{n\to\infty}P(|X-X\)n| < \varepsilon)=1$,则称**随机变量序列**$\\{X\_n\\}$**依概率收敛于**随机变量$X$,记为$X\_n\overset{P}{\to}X$
